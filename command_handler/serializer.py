...
from command_handler.utils import NULL_RESP
...
class ArraySerializer(SerializationStrategy):
    def serialize(self, value):
        ...
        serialized_values = [SerializorFactory.create_serializor(type(val)).serialize(val)
                             for val in value]
...
class SerializorFactory:
    ...
    def create_serializor(value: SerializerInputType) -> SerializationStrategy:
        print(f"value {value}")
        strategy: SerializationStrategy | Callable = SerializorFactory.strategy_types.get(value)
